...
```bash
python3 -m http.server 8742 --directory .autoresearch/[skill-name] &
...
**IMPORTANT: browsers block `fetch()` on `file://` URLs (CORS restriction).** The server was started in step 1. `results.json` is served over HTTP so `fetch()` works and the 10-second auto-refresh is live. Do NOT use `open dashboard.html` directly — that loads via `file://` and the dashboard will appear empty.
...
1. Create `results.tsv` with the header row (the directory already exists from step 1)
2. Back up the original SKILL.md as `SKILL.md.baseline`
3. Run the skill [N] times using the test inputs
4. Score every output against every eval
5. Record the baseline score and update both results.tsv and results.json
